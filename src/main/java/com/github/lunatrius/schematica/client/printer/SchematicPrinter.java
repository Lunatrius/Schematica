...
import aceart.update.BoxBuilder;
...
public class SchematicPrinter {
	...
	public RayTraceResult getBlockBeingLookedAt() {
...
//		final Minecraft minecraft = Minecraft.getMinecraft();
	...
	private boolean blockIsReachable(BlockPos blockToPlace, final Vector3d playerPos, double reachDistance) {
		...
		if(blockToPlace.distanceSqToCenter(playerX, playerY, playerZ) > Math.pow(reachDistance, 2)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean print(final WorldClient world, final EntityPlayerSP player) {
		...
		RayTraceResult tracedBlock = getBlockBeingLookedAt(); 
		BlockPos centralBlock = tracedBlock.getBlockPos();
		EnumFacing blockSide = tracedBlock.sideHit;
...
//		final double dZ = ClientProxy.playerPosition.z - this.schematic.position.z;
		final double dX = centralBlock.getX() - this.schematic.position.x;
		final double dY = centralBlock.getY() - this.schematic.position.y;
		final double dZ = centralBlock.getZ() - this.schematic.position.z;
		
		
		
		Vec3i schematicStart = new Vec3i(this.schematic.position.x,
				this.schematic.position.y, this.schematic.position.z);
		...
		final double playerX = ClientProxy.playerPosition.x - this.schematic.position.x;
...
//		final double blockReachDistanceSq = blockReachDisytance * blockReachDistance;
		...
		Iterable<MBlockPos> box = BlockPosHelper.getAllInBoxXZY(minX, minY, minZ, maxX, maxY, maxZ);
		
		EnumFacing playerFacing = ClientProxy.orientation;

//		BoxBuilder builder = new BoxBuilder(relativeCentral, playerFacing);
//		builder.buildBox();
//		ArrayList<MBlockPos> box = builder.getBox();
//		System.out.println("box size: " + box.size());
		
//		int i = 0;
		ArrayList<BlockPos> listTemp = new ArrayList<>();
//		for(BlockPos pos : box) {
//			listTemp.add(pos);
//			i++;
//		}
//		System.out.println("ITerable box size:" + i);
		
		String str1 = null;
		java.util.Iterator<MBlockPos> it = box.iterator();
		while(it.hasNext())
			str1 += "\n" + it.next().add(schematicStart);
		
		String str = null;
		
		for (BlockPos pos : box) {

//			if (!this.schematic.isInside(pos) || !blockIsReachable(pos, ClientProxy.playerPosition, 
//					reachDistance)) {
//				continue;
//			}
